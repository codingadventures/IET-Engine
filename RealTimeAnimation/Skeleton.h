...
public:
	...
	void skeleton_animate_with_animation  (
		...
		glm::mat4* bone_animation_mats
			...
			if (node->num_pos_keys > 0) {
				...
				node_T = glm::translate(glm::mat4(), lerped);
			}
	...
	Bone* findNodeInSkeleton (const char* node_name, Bone* boneToFind) {
		...
		// recurse to children
		for (int i = 0; i < boneToFind->numChildren; i++) {
