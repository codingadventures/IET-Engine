...
{	
	...
	for (int i=0;i<deviceCount;i++)
	{
		...
		int spCores = _ConvertSMVer2Cores(dev.major, dev.minor) * dev.multiProcessorCount;

#if CUDA_VERSION >= 2010
		printf("CUDA:  %d : \"%s\", %d MB, %d cores at %.3f GHz, revision %d.%d",i,dev.name, dev.totalGlobalMem/(1024*1024), spCores, dev.clockRate * 1e-6f, dev.major, dev.minor);
		if (dev.kernelExecTimeoutEnabled)
			printf(", timeout enabled", dev.kernelExecTimeoutEnabled);
		printf("\n");
#elif CUDA_VERSION >= 2000
		printf("CUDA:  %d : \"%s\", %d MB, %d cores at %.3f GHz, revision %d.%d\n",i,dev.name, dev.totalGlobalMem/(1024*1024), spCores, dev.clockRate * 1e-6f, dev.major, dev.minor);
#else //if CUDA_VERSION >= 1000
		printf("CUDA:  %d : \"%s\", %d MB, cores at %.3f GHz, revision %d.%d\n",i,dev.name, dev.totalGlobalMem/(1024*1024), dev.clockRate * 1e-6f, dev.major, dev.minor);
		//#else
		//		printf("CUDA:  %d : \"%s\", %d MB, revision %d.%d\n",i,(dev.name==NULL?"":dev.name), dev.bytes/(1024*1024), dev.major, dev.minor);
#endif
	}
	if (device==-1)
	{
	...
	{
		printf("CUDA: Device %d not found.\n", device);
		return 0;
	}
	else
	{
...
void mycudaMalloc(void **devPtr, size_t size,int /*d*/)
{
	...
	if (mycudaVerboseLevel>=LOG_INFO) printf("CUDA: malloc(%d).\n",size);
	cudaCheck(cudaMalloc(devPtr, size),"cudaMalloc");
	if (mycudaVerboseLevel>=LOG_TRACE) printf("CUDA: malloc(%d) -> 0x%x.\n",size, *devPtr);
}

void mycudaMallocPitch(void **devPtr, size_t* pitch, size_t width, size_t height)
{
	if (!cudaInitCalled) mycudaInit();
	if (mycudaVerboseLevel>=LOG_INFO) printf("CUDA: mallocPitch(%d,%d).\n",width,height);
	cudaCheck(cudaMallocPitch(devPtr, pitch, width, height),"cudaMalloc2D");
	if (mycudaVerboseLevel>=LOG_TRACE) printf("CUDA: mallocPitch(%d,%d) -> 0x%x at pitch %d.\n",width,height, *devPtr, (int)*pitch);
}

void mycudaFree(void *devPtr,int /*d*/)
{
	if (mycudaVerboseLevel>=LOG_TRACE) printf("CUDA: free(0x%x).\n",devPtr);
...
{
	if (!cudaInitCalled) mycudaInit();
	if (mycudaVerboseLevel>=LOG_TRACE) printf("CUDA: mallocHost(%d).\n",size);
	cudaCheck(cudaMallocHost(hostPtr, size),"cudaMallocHost");
	if (mycudaVerboseLevel>=LOG_TRACE) printf("CUDA: mallocHost(%d) -> 0x%x.\n",size, *hostPtr);
}

void mycudaFreeHost(void *hostPtr)
{
	if (mycudaVerboseLevel>=LOG_TRACE) printf("CUDA: freeHost(0x%x).\n",hostPtr);
...
void mycudaMemcpyHostToDevice(void *dst, const void *src, size_t count,int /*d*/)
{
	if (!cudaCheck(cudaMemcpy(dst, src, count, cudaMemcpyHostToDevice),"cudaMemcpyHostToDevice"))
		printf("in mycudaMemcpyHostToDevice(0x%x, 0x%x, %d)\n",dst,src,count);
}
...
{
	if (!cudaInitCalled) mycudaInit();
	printf("mycudaGLRegisterBufferObject %d\n",id);
...
void cuda_void_kernel()
{   
	printf("WARNING : cuda_void_kernel should only be use for debug\n");
...
{
   printf("CUDA error: %s returned from %s\n", err , src);
    exit(1);
}
